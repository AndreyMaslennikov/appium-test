...
let ShowBranchBuildStatusCommand = class ShowBranchBuildStatusCommand extends commandline_1.AppCommand {
    run(client, portalBaseUrl) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let builds;
            try {
                builds = yield interaction_1.out.progress(`Getting builds for branch ${this.branchName}...`, client.builds.listByBranch(this.branchName, app.ownerName, app.appName));
            }
            catch (error) {
            ...
            let commits;
            try {
                commits = yield interaction_1.out.progress(`Getting commit info for ${lastBuild.sourceVersion}...`, client.commits.listByShaList([lastBuild.sourceVersion], app.ownerName, app.appName));
            }
            catch (error) {
                debug(`Request failed - ${util_1.inspect(error)}`);
                return commandline_1.failure(commandline_1.ErrorCodes.Exception, "the Branch Builds request was rejected for an unknown reason");
