        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (this.principalType === principal_type_1.PrincipalType.USER) {
                    yield interaction_1.out.progress(tokenMessaging, client.userApiTokens.delete(this.id, {
                    ...
                    const app = this.app;
                    yield interaction_1.out.progress(tokenMessaging, client.appApiTokens.delete(app.ownerName, app.appName, this.id, {
                        onResponse: (response, _flatResponse, _error) => this.handleCreateTokenResponse(response.status),
                    }));
                }
            }
    ...
    handleCreateTokenResponse(statusCode) {
        if (statusCode >= 400) {
            switch (statusCode) {
                ...
                case 404:
                    throw commandline_1.failure(commandline_1.ErrorCodes.NotLoggedIn, `the ${this.principalType} API token with ID "${this.id}" could not be found`);
            }
        }
    }
};
