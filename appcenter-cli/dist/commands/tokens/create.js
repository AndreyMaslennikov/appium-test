        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (this.principalType === principal_type_1.PrincipalType.USER) {
                    createTokenResponse = yield interaction_1.out.progress(tokenMessaging, client.userApiTokens.create(tokenAttributes));
                }
                else if (this.principalType === principal_type_1.PrincipalType.APP) {
                    const app = this.app;
                    createTokenResponse = yield interaction_1.out.progress(tokenMessaging, client.appApiTokens.create(app.ownerName, app.appName, tokenAttributes));
                }
            }
            catch (error) {
                return error;
            }
            format_token_1.reportToken(createTokenResponse);
            return commandline_1.success();
        });
    }
    handleCreateTokenResponse(statusCode) {
        if (statusCode >= 400) {
            switch (statusCode) {
                case 400:
                default:
                    throw commandline_1.failure(commandline_1.ErrorCodes.Exception, "invalid request");
                case 403:
                    throw commandline_1.failure(commandline_1.ErrorCodes.InvalidParameter, "authorization to create an API token failed");
                case 404:
                    throw commandline_1.failure(commandline_1.ErrorCodes.NotLoggedIn, `${this.principalType} could not be found`);
            }
        }
    }
};
