        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (this.principalType === principal_type_1.PrincipalType.USER) {
                    listTokensResponse = yield interaction_1.out.progress(tokenMessaging, client.userApiTokens.list());
                }
                else if (this.principalType === principal_type_1.PrincipalType.APP) {
                    const app = this.app;
                    listTokensResponse = yield interaction_1.out.progress(tokenMessaging, client.appApiTokens.list(app.ownerName, app.appName));
                }
            }
            catch (error) {
            ...
            interaction_1.out.table(interaction_1.out.getCommandOutputTableOptions(["ID", "Description", "Type", "Created At"]), listTokensResponse.map((apiToken) => [apiToken.id, apiToken.description, this.principalType, apiToken.createdAt]));
